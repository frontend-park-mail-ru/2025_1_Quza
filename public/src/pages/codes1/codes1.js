...
export default class CodesPage1 extends Page {
    ...
    for (const code of codes) {
      if (code.id === 'codes1' && code.data.length > 0) {
        for (const block of code.data) {
          let te = new TextContainer(
            document.querySelector('.codes-container1'),
            { content: block },
          );
          te.render();
        }
        let hasVerticalScrollbar =
          this.#codesContainer.scrollHeight > this.#codesContainer.clientHeight;
  ...
  createObserver() {
    ...
    observer.observe(
      this.#codesContainer.querySelector('.code-container1:last-child'),
    );
  }
  ...
  #unsubscribeFromEvents() {
    AppEventMaker.unsubscribe(
      CodesStoreEvents.CODES_RECEIVED,
      this.#renderCodes,
    );
  }
